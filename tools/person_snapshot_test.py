...
MUTATIONS = [
    ("accept-oversized-native-item-string",
     "if (item.hasModData()) checkNativeTable(item.getModData(), 0, new IdentityHashMap<>());",
     "", "oversized native item string accepted"),
    ("living-capture-obsolete-equipment",
     "requireLiving(living);\n        ItemContainer empty = new ItemContainer();",
     "requireLiving(living);\n        equipment(living, new LinkedHashMap<>());\n        ItemContainer empty = new ItemContainer();",
     "Equipment outside inventory"),
    ("return-historical-inventory",
     "serialize(buffer -> current.save(buffer)),",
     "serialize(buffer -> living.getInventory().save(buffer)),",
     "Restored item identity/type/container differs"),
    ("return-dropped-detached-equipment", "view.getItems().add(item);", "",
     "Equipment outside inventory"),
    ("return-premature-item-processing", "return restore(shell, packed, false);",
     "return restore(shell, packed, true);", "detached return item processed before activation"),
    ("return-source-change-ignored", "return Arrays.equals(snapshot.sections()[0],",
     "return true || Arrays.equals(snapshot.sections()[0],", "changed source materials accepted"),
    ("return-visual-not-restored", "destination.getVisual().load(buffer, VERIFIED_WORLD_VERSION);",
     "buffer.position(buffer.limit());", "return appearance did not roundtrip"),
