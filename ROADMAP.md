...
reconciliation. The next unit proves actor-specific access and performed use
across R6, R7 and R9. Decision/action separation, social producers and
presentation follow in dependency order. Existing R work moves with its owning
contract.
...
native-ground producer; R6, R7 and R9 now own exact interaction access,
transfer and performed-use proof. R11-R12 can proceed alongside them.
...
| **R6. Perception and access — SAO (loaded repair closed C60; native ground closed C61; actor proof open)** | R2 for timestamps; C61 source observations | Select one exact source interaction point and prove current source revision, floor, route, barriers and doors, claim or ownership permission, and vehicle-part permission. A person's private source belief can make the target known; only those current physical facts make it accessible. Unknown and unsupported ground remains unavailable. |
| **R7. Action ownership and receipts — SAO; ZAO retained verbs** | R1-R2, R4, R6 | Connect intent, approach, queue, execution, interruption and observed result. For C61's source continuation, transfer the exact reserved item or fluid to the actor, invoke carried native `Eat` or `DrinkFluid`, and publish one durable exact-once result tied to the source revision. Cancellation releases ownership; reload preserves or reconciles pending work; a changed revision conflicts. Crossed actions use retained learned capability and the shared action contracts; the Afflicted exposure path has its own non-feeding result. Inventory projections, experience and relationships update only from the appropriate result. |
...
| Unloaded world opportunity | Closed in C61 for native grounding: demand-led streamer hydration, native loot fill, persistent UUID source identity, exact bounded private observations, replacement/movement conflicts and loaded reconciliation. The next named output is one actor-specific access-and-action proof covering exact interaction point, route/floor/doors, claims and vehicle permission, exact transfer, native carried consequence, durable revision-tied result, interruption and reload. | R6, R7, R9 |
