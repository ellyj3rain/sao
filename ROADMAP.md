...
cross-repository join boundary; executable options, choices and consequences
remain R11 work. Existing R work moves with its owning contract.
...
projection with later native reconciliation. R9 remains open for shelving and
the other provisioning/development producers. C64 closes R11's immutable
...
| **R11. Decision capture and joins — SAO + Speakeasy (envelope/join integrity closed C64)** | Start now; final eligibility follows R2, R6-R10 | C64 freezes immutable person/situation evidence before the real election, attaches its result separately, records exact run/county/event identity, horizons, settings and provenance, refuses reader/callback/incomplete-run failures and publishes all counties atomically. Speakeasy v3 prevalidates exact namespaced joins and protected destinations. The legacy election exposes no executable options or model choice, so option capture, separate choice authoring, execution revalidation and action-specific consequence horizons remain open. |
| **R12. Approved data and world knowledge — Speakeasy; SAO knowledge reader (protection/audit closed C64)** | Start now; R11 for new captures | Speakeasy v3 hash-protects 190 approved choices, four historical derivatives and nine approved world documents. C64 audits conditioning independently: future state contaminates most legacy rows and all options are bare strings, so none are training-eligible. Build versioned decision-time corpus/claim views and person-specific acquisition. Future facts, LOW claims and lineage leakage are refused; historical corrections remain append-only. |
...
| Unloaded world opportunity | Closed through C64 for the selected source path: C61 supplies demand-led native grounding, C62 supplies private revision selection, current route/floor/door/claim/vehicle access, exact native transfer, carried consequence, interruption, reload and durable result, and C63-C64 attribute and reconcile only that exact partial source before acknowledging. Aggregate house claims/storage now wait for complete evidence or their own performed producers. The remaining work is shelving and the other R7/R9 action families. | R6 closed; R7 source slice closed; R9 exact partial-source projection slice closed |
