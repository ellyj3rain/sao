...
policies and no SAO-inferred maintenance. ZAO A39 supplies Afflicted water,
...
C84/ZAO A41 move appraisal into shared loaded/dormant owners and wrap the
production county in a single-prefix, fresh-process exact-replay episode. The
tracked 30-day run preserves a zero-decision result and Speakeasy Record 68
...
| **R15. Late-start acceleration — SAO + Speakeasy (baseline episode boundary C84/Record 68)** | R10b; versioned policy/export from R13-R14 | C84 supplies single-prefix checkpoints, complete source provenance, isolated reset, exact replay and exclusive episode export; Record 68 preserves each episode as the provenance unit before any decision/task extraction. Evaluate a future accelerator against multiple repaired causal episodes with matched seeds, initial conditions and policy version. Compare histories, distributions and subsequent behavior, preserve provenance and quantify error/performance. The current 30-day seed contains no matters and is not a sufficient corpus or equivalence study. Retain causal stepping until a candidate passes; fixed survival floors, settlement quotas and authored completed work remain invalid. |

C81's production scene source emits exact v3 rows and keeps response
...
Communication and registered ZAO driver boundaries. Speakeasy Record 66 supplies
their independent task review and bounded admission; Record 67 and C82 supply
exact native export plus non-authoritative consumption. R11 still requires
...
| R13 task and retrieval examples | Extend beyond the bounded Record 66 coordination family to independently sourced understander, retriever and speaker datasets with lineage-separated evaluation. | Records 53-67 settle label, admission, tokenizer, reference and export mechanics; broader eligible examples and task-specific quality evidence remain required. |
| R14 conversation runtime | Extend C82's coordination shadow to the shared understander/retriever/speaker bundle, then present consequential activation separately. | C82 proves packaged inference, bounded async ownership and stale/foreign/current-owner refusal without behavior authority. Loaded-game memory/latency and actual exchange quality remain required. |
