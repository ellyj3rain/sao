...
-- [R10a] Reconcile the player's loaded ground with the exact source ledger.
-- Unopened containers remain unknown; native item IDs, fluid quantities and
-- removal are observed without converting them to a building visit count.
local LOOT_REACH = 8
...
function Age.playerLoots(player)
    ...
    local changed = 0
    pcall(function()
        changed = SAO.WorldSources.observeAt(x, y, LOOT_REACH)
    end)
    return changed or 0
end
