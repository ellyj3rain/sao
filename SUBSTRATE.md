...
| Loaded and dormant opportunities describe the same world | `SAO_WorldSources` requests bounded native chunks, records exact revisions and reconciles loaded changes; `SAO_SourceUse` turns one actor's private revision into a two-leg approach, current permission proof, exact native transfer and carried native use; `SAO_Provisioning` projects completed material effects and returns explicitly commitment-bound terminal failures without projection; `SAOPrivateInventory` gives current decisions exact recursive holder rows | Native UUIDs and item IDs; bounded observations/conflicts and a schema-6 action/ambient-change store; one live reservation per actor; bounded ordered terminal receipts; durable partial `Material` source projection, replay decision, projection generation and single-source owner index; schema-4 Standing migration retires inferred totals; fresh actor-private loaded views and exact v4 dormant carriage; runtime-only engine bindings | C61 Border 178 proves native grounding. C62 Border 179 runs the executable lifecycle. C63-C64 Border 180 executes the projection contract and C71 extends it with complete-coverage refusal. C71 Border 184 proves recursive holder conservation, removal/replacement, transfer, reload and loaded/dormant agreement. C85 Border 197 executes coordinated acquisition, carrying, handover, terminal failure and exact-once consumption. | R6 is closed for the selected source path, R7's source-action slice is complete and R9's exact partial-source projection slice is closed. C66 adds exact native acquisition/storage and completion-only experience. C71 closes actor-private inventory observation; C85 closes one natural provisioning producer and execution chain. Complete household stock, dormant world holders and other action/producer families remain open. |
...
| Social outcomes follow acquired proposals, participant responses and performed acts | C79 Organization owns versioned processes, receptions, private responses, scoped commitments and work receipts; C85 Perception, dormant/loaded controllers, Communication and Coordination originate private situations, contact particular retained addresses and perform accepted work through SourceUse, Locomotion and Handover | Graph schema 4 stores organizations, offices, revision-safe claim/decision history, processes, contact attempts, participants, commitments and consumed result IDs | Border 190 executes all response kinds plus unanswered, revision/scope and reload. Borders 194-196 execute external appraisal, production pressure, contact waiting and causal capture. Border 197 joins acquisition/carry/delivery, pause/failure/partial/completion and exact-once receipt consumption. | One natural provisioning/contact/work producer is implemented. Elections, deliberation, succession, coercion, physical separation, care, development, wider institutions and the remaining R9 producers remain open. A roster, score, generic message, address arrival or selection still cannot create recognition. |
...
| Training sees the actual decision moment | C79 `Organization.decisionEvidence` reconstructs one recipient's response-formation horizon by process revision and exposes later delivery/work separately; C81 runs pre-partitioned situations through that owner while Speakeasy Record 63 requires the exact v3 namespace and actor/executor/source ownership | Durable process history remains in GraphPersistence; detached task evidence binds run/county/person/event/hour, proposal revision, feasible responses, choice and later commitment receipts | Border 190 proves decision/outcome separation and retrieval of revision 1 after revision 2. Border 191 reproduces twenty source rows and mutates Controller pressure selection, ZAO registration and Driver attribution. Speakeasy controls reject wrong person, stale revision, impossible option, hidden diagnosis and outcome leakage. | Record 66 independently reviews and admits the exact five-response synthetic family across Survivor, Afflicted and Crossed owners. Decline/withdraw examples, loaded sampling and broader action coverage remain open. |
| Learned cognition reaches consequences | C82 packages Record 67's exact FP32 coordination artifact and evaluates immutable post-appraisal snapshots in a bounded Java worker; Lua revalidates response/process/options and current registered owner before retaining a shadow observation | Bundle/model source identities are content-hashed; worker and observations are runtime-only and reset on world bind/detach | Border 192 executes all 20 Python/Java vectors, six tokenizer cases, malformed/mask/capacity/reset controls and the shipped jar. Border 193 executes authoritative response ordering plus Survivor/shared-ZAO paths and stale/foreign/hidden/owner-drift refusal. | The shadow has no authority and no persistence. Behavior activation, loaded-game cost/quality acceptance and the shared understander/retriever/speaker bundle remain open; an observation is not the promised consequence. |
...
| Dead, living and Afflicted interaction | Crossed may hunt and feed on eligible ordinary humans, but cannot feed on Afflicted provenance. Ordinary food supplies ordinary human calories; human origin remains motivationally preferred rather than biologically required. Afflicted food policy remains separately owned and unchanged here. Intentional exposure has its own exact-once pathogen action. | Complete strategy, diversion, capture, mutilation, group precedent and use of the dead remain per-concern producers. Eating, killing and exposure remain separate results. |
